...
class CrowdSimConfig:
    ...
    - For each cluster ``g``, sample a cluster-specific confusion matrix
      ``C[a,g]`` around ``C_base[a]`` via a Dirichlet centered at C_base[a]
      with concentration ``kappa_a``.
...
def sample_cluster_confusions(
    *,
    ...
    Model:
      C[a,g,t,:] ~ Dirichlet(kappa[a] * C_base[a,t,:])
    ...
    for a in range(A):
        ...
        for g in range(G):
            for t in range(K):
                alpha = np.clip(ka * base[a, t], eps, None)
